...
Quadrotor::Quadrotor(ros::NodeHandle& nh) : trajectory_client("/action/trajectory",true)
{
    ...
    tour_sub = nh.subscribe<gtsp::Tour>("/gtsp_tour_list",1,&Quadrotor::tourCallback,this);

    point_cloud_publisher = nh.advertise<sensor_msgs::PointCloud2>("/gtsp_point_cloud", 1);
    gtsp_tour_ready = false;

    frontier_cloud.reset(new pcl::PointCloud<pcl::PointXYZ>);
...
void Quadrotor::tourCallback(const gtsp::Tour::ConstPtr & msg)
{
    ROS_INFO("GTSP Tour ready");
    gtsp_tour = msg->tour;
    gtsp_tour_ready = true;
}
...
void Quadrotor::findFrontier()
{
    ...
    if(planning_scene_service.call(srv)){
        ...
        ROS_INFO("Creating frontier point cloud");
        for(int i=0;i<indices.size();i++)
        {
            frontiers.push(candidate_frontiers[i]);
            pcl::PointXYZ point(candidate_frontiers[i].second.position.x, candidate_frontiers[i].second.position.y, candidate_frontiers[i].second.position.z);
...
void Quadrotor::run()
{
    ...
    while(ros::ok()){
        ...
        pcl::toPCLPointCloud2(*frontier_cloud, pcl_pc);
        sensor_msgs::PointCloud2 output;
        pcl_conversions::fromPCL(pcl_pc, output);

        ROS_INFO("Wait for GTSP node to subscribe");
        ...
        ROS_INFO("Sending points to GTSP module");
        gtsp_tour_ready = false;
        point_cloud_publisher.publish(output);
        while(!gtsp_tour_ready)
        {
            ros::spinOnce();
            rate.sleep();
        }
        ...
        tree_->setInputCloud(frontier_cloud);

        std::vector<int> nn_indices (1);
        std::vector<float> nn_dists (1);

        tree_->nearestKSearch(pcl::PointXYZ(odometry_information.position.x, odometry_information.position.y, odometry_information.position.z), 1, nn_indices, nn_dists);
        ROS_INFO("Closest point: (%f, %f, %f)", frontier_cloud->points[nn_indices[0]].x, frontier_cloud->points[nn_indices[0]].y, frontier_cloud->points[nn_indices[0]].z);

        int index = -1;
        std::vector<int>::iterator it;
        for(it = gtsp_tour.begin(); it != gtsp_tour.end(); ++it)
        {
            if((nn_indices[0] + 1) == *it)
                break;
        }

        for(int i = 0; i < gtsp_tour.size(); i++)
        {
            geometry_msgs::Pose _goal;
            _goal.position.x = frontier_cloud->points[*it-1].x;   
            _goal.position.y = frontier_cloud->points[*it-1].y;
            _goal.position.z = frontier_cloud->points[*it-1].z;
            ...
            ROS_INFO("Try to go to point: (%f, %f, %f)", frontier_cloud->points[*it-1].x, frontier_cloud->points[*it-1].y, frontier_cloud->points[*it-1].z);
            success = go(_goal);
